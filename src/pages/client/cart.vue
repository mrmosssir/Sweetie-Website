  ...
  <div>
    ...
    <div class="flex flex-col gap-4 md:flex-row">
        ...
        <li v-for="item in cartProducts" :key="item.id" class="grid grid-cols-1 md:grid-cols-4 text-gray-700 border-b border-gray-200 py-6">
          <div class="flex items-center gap-x-4 col-span-1 md:col-span-2">
            ...
            <div class="flex flex-col gap-y-2">
              ...
              <span class="inline md:hidden text-sm text-gray-700 mt-2">單價 : {{ item.price.toFixed(2) }}</span>
          ...
          <div class="hidden md:flex justify-center items-center text-sm text-gray-700">${{ item.price.toFixed(2) }}</div>
        </li>
      </ul>
      <div class="flex flex-col gap-y-4 ml-auto w-full p-6 border border-gray-200 h-fit md:w-80">
        ...
        <button class="bg-[#477182] text-white text-center py-3 mt-4 cursor-pointer" @click="handleSubmit">前往結帳</button>
      </div>
    </div>
  </div>
</template>
...
const handleGetProductsDetail = async () => {
  ...
  cartProducts.value = products.map((item) => {
    const quantity = cartStore.carts.find((cartItem) => cartItem.productId === item.id)?.quantity || 1;
    return {
      ...item,
      quantity,
