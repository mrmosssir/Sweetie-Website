<template>
  <div class="w-full max-w-64 mx-auto">
    <div class="relative bg-cover bg-center bg-no-repeat w-full aspect-[4/5]" :style="{ backgroundImage: `url(${product.image})` }">
      <div class="flex flex-col gap-y-2 absolute top-0 right-0 p-3">
        <button class="bg-white w-9 h-9 rounded-full flex items-center justify-center cursor-pointer" @click="handleFavorite(isFavorite)">
    ...
    <button class="bg-gray-200 text-gray-700 px-2 py-0.5 cursor-pointer mt-2" @click="cartStore.addToCart(product.id)">加入購物車</button>
  </div>
</template>
